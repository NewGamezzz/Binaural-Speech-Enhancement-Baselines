...
class ConvTasNet(torch.nn.Module):
    ...
    *Conv-TasNet: Surpassing Ideal Time–Frequency Magnitude Masking for Speech Separation*
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if input.ndim != 3 or input.shape[1] != 2:
            raise ValueError(
                f"Expected 3D tensor (batch, channel==2, frames). Found: {input.shape}"
            )
        ...
        input_L, input_R = input[:, 0].unsqueeze(1), input[:, 1].unsqueeze(1)
        padded_L, num_pads = self._align_num_frames_with_strides(input_L)  # B, 1, L'
        padded_R, num_pads = self._align_num_frames_with_strides(input_R)  # B, 1, L'
        batch_size, num_padded_frames = padded_L.shape[0], padded_L.shape[2]
        feats_L = self.encoder(padded_L)  # B, F, M
        feats_R = self.encoder(padded_R)
        feats = torch.cat([feats_L, feats_R], dim=1)
        masked = self.mask_generator(feats) * feats.unsqueeze(1)  # B, S, F, M
        masked = masked[:, :, : self.enc_num_feats] + masked[:, :, self.enc_num_feats :]
        ...
        # assert input_shape == output.shape, "Output must have the same shape as input."
        return output


def conv_tasnet_base(num_sources: int = 1) -> ConvTasNet:
