...
class GaGNetModule(nn.Module):
    ...
    def forward(self, x):
        # Preprocess x (time-domain)
        with torch.no_grad():
            _, length = x.shape
            norm_factor = x.abs().max().item()
            x = x / norm_factor
            x = torch.stft(x, **self.istft_kwargs, return_complex=True)
            x = self.spec_fwd(x)
            x = x.unsqueeze(1)
            x = pad_spec(x)
        ...
        out = out.squeeze(1)
        out = self.spec_back(out)
        out = torch.istft(out, **self.istft_kwargs, length=length)
        out = out * norm_factor
        return out
