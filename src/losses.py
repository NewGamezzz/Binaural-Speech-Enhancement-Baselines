...
class BinauralLoss(nn.Module):
    ...
    def forward(self, model_output, targets):
        ...
        if self.ild_weight > 0:
            ild_loss = ild_loss_db(
                target_stft_l.abs(), target_stft_r.abs(), output_stft_l.abs(), output_stft_r.abs()
            )
        ...
        if self.ipd_weight > 0:
            ipd_loss = ipd_loss_rads(target_stft_l, target_stft_r, output_stft_l, output_stft_r)
